...
async fn main(spawner: Spawner) {
...

    {
        ...
        let mut my_file = match root_dir.open_file_in_dir("MY_FILE.TXT", embedded_sdmmc::Mode::ReadOnly) {
            Ok(file) => file,
            Err(err) => {
                let mut text: heapless::String<64> = heapless::String::new();
                write!(&mut text, "We got an error: {:?}\r\n", err).unwrap();
    ...
    }
    {
        // Open a file called "MY_FILE.TXT" in the root directory
        // This mutably borrows the directory.
        let mut my_file = match root_dir.open_file_in_dir("MY_FILE.TXT", embedded_sdmmc::Mode::ReadWriteAppend) {
            Ok(file) => file,
            Err(err) => {
                let mut text: heapless::String<64> = heapless::String::new();
                write!(&mut text, "We got an error: {:?}\r\n", err).unwrap();
